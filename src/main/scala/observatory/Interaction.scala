...
object Interaction extends InteractionInterface {
  ...
  val power_num = 8
  val alpha = 127
  val width = pow(2, power_num).toInt
  val height = pow(2, power_num).toInt
  ...
  def tile(temperatures: Iterable[(Location, Temperature)], colors: Iterable[(Temperature, Color)], tile: Tile): Image = {
    val offX = (tile.x * pow(2, power_num)).toInt
    val offY = (tile.y * pow(2, power_num)).toInt
    val offZ = tile.zoom
    val coordinates = for {
      i <- 0 until height
      j <- 0 until width
    ...
    val pixels = coordinates.par
      .map({case (y, x) => Tile(x + offX, y + offY, power_num + offZ)})
      .map(tileLocation)
